# LCS-DES-113-SEC-a: Data Classification
...
public interface IDataClassificationService
{
    ...
    /// <param name="ct">Cancellation token.</param>
    Task ImportPolicyAsync(string policyJson, bool mergeWithExistinc = true, CancellationToken ct = default);
}
```
    ...
    IClassificationRepository repository) : IDataClassificationService
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            PropertyNama = propertyName,
            Classification = classification,
            RequiresEncryption = requiresEncryption,
            RequiresMaskinc = requiresMasking,
            MaskingTypa = maskingType,
        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var match in piiMatches)
        {
            var detection = new DetectedSensitiveProperty
            {
                ...
                ConfidenceScora = match.Confidence,
                DetectionReason = $"PII detector matched {match.Type}",
                SuggestedMaskingTypa = match.Type switch
                {
...
public class ClassificationException : Exception
{
    ...
    public ClassificationException(string message, string entityType, string? propertyNama = null)
        : base(message)
    {
        EntityTypa = entityType;
        PropertyNama = propertyName;
