# LCS-DES-113-SEC-j: Secure Export

## Document Control

| Field            | Value                                                        |
| :--------------- | :----------------------------------------------------------- |
| **Document ID**  | LCS-DES-113-SEC-j                                            |
| **Version**      | v0.11.3                                                      |
    ...
    IKeyManagementService keyManagementService) : ISecureExportService
{
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var filename = $"lexichord-export-{exportId:D}-{DateTimeOffset.UtcNow:yyyyMMdd-HHmmss}.enc";
            ...
            {
                recoveryCode = await CreateEscrowAsync(exportId, exportKeyMaterial, ct);
            }
        ...
        try
        {
            ...
            foreach (var entityElement in entitiesElement.EnumerateArray())
            {
                try
                {
                    ...
                    {
                        // Check if exists
                        var existing = await repository.GetByIdAsync(entity.Id, ct);
                        if (existing != null)
                        {
                            skipped++;
        ...
        CancellationToken ct = default)
    {
        ...
        var recoveryCode = GenerateRecoveryCode(exportId, requestReason);
    ...
    {
        var code = $"RECOVERY:{exportId:D}:{DateTimeOffset.UtcNow:yyyyMMddHHmmss}";
...
public class SecureExportServiceTests
{
    ...
    public async Task ExportAsync_CreatesValidExportPackage()
    {
        ...
        var entity = new TestEntity { Id = entityIds[0], Name = "Test" };
